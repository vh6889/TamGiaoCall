...
if (is_manager()) {
    $users = db_get_results("
        ...
        LEFT JOIN orders o ON u.id = o.assigned_to AND o.status NOT IN (SELECT status_key FROM order_status_configs WHERE label LIKE '%mới%' OR label LIKE '%hoàn%' OR label LIKE '%hủy%')
        WHERE ma.manager_id = ?
        GROUP BY u.id
